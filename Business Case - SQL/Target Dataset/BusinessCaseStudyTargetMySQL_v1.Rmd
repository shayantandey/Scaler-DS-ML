...
7.8. The sales trend in the dataset shows gaps in the plot. This suggests that data  
     is not available for all days.
...
ggplot(sales_dataAll, aes(x = day, y = order_count)) +
  ...
  labs(title = "Orders in the dataset", x = "Sep 2016 - Oct 2018", y = "Order Count") +
  theme_minimal()

```

```{sql echo=TRUE, connection=target_db}
WITH d AS (SELECT DISTINCT DATE(order_purchase_timestamp) as day,
COUNT(order_id) OVER (PARTITION BY DATE(order_purchase_timestamp)) AS order_count
FROM orders
ORDER BY DATE(order_purchase_timestamp))
SELECT max(day), min(day) FROM d
