...
```
Analyzing the *order_status* we find the following status. Including or excluding orders with certain status will change the percentage increase.

```{sql, connection=target_db}
SELECT DISTINCT order_status FROM orders;
```

Foe example, excluding the orders with *canceled* status changes the results as follows:
...
SELECT 
  d.year, 
  d.month, 
  COUNT(CASE WHEN d.payment_type = 'credit_card' THEN d.order_id ELSE NULL END) 
        AS credit_card,
  COUNT(CASE WHEN d.payment_type = 'UPI' THEN d.order_id ELSE NULL END) 
        AS UPI,
  COUNT(CASE WHEN d.payment_type = 'voucher' THEN d.order_id ELSE NULL END) 
        AS voucher,
  COUNT(CASE WHEN d.payment_type = 'debit_card' THEN d.order_id ELSE NULL END) 
        AS debit_card,
  d.month_number
FROM d
...
query <- "SELECT DISTINCT LEFT(MONTHNAME(order_purchase_timestamp), 3) as month,
       COUNT(order_id) OVER (PARTITION BY MONTH(order_purchase_timestamp))
        AS order_count,
       MONTH(order_purchase_timestamp) as month_number
FROM orders
WHERE YEAR(order_purchase_timestamp) = 2016
ORDER BY MONTH(order_purchase_timestamp);"
sales_data <- dbGetQuery(target_db, query)
all_months <- data.frame(month = factor(month.abb, levels = month.abb))

sales_data <- all_months %>%
  left_join(sales_data, by = "month") %>%
...
# ggplot(sales_data, aes(x = month, y = order_count)) +
#  geom_bar(stat = "identity", fill = "steelblue") +
#  labs(title = "Orders in 2016", x = "2016", y = "Order Count")
```
...
# ggplot(sales_data, aes(x = month, y = order_count)) +
#  geom_bar(stat = "identity", fill = "steelblue") +
#  labs(title = "Orders in 2017", x = "2017", y = "Order Count")
```
...
# ggplot(sales_data, aes(x = month, y = order_count)) +
#  geom_bar(stat = "identity", fill = "steelblue") +
#  labs(title = "Orders in 2018", x = "2018", y = "Order Count")
```

```{r echo = FALSE, results = TRUE}
query2016 <- "SELECT DISTINCT LEFT(MONTHNAME(order_purchase_timestamp), 3) as month,
       COUNT(order_id) OVER (PARTITION BY MONTH(order_purchase_timestamp))
        AS order_count,
       MONTH(order_purchase_timestamp) as month_number
FROM orders
...
query2017 <- "SELECT DISTINCT LEFT(MONTHNAME(order_purchase_timestamp), 3) as month,
       COUNT(order_id) OVER (PARTITION BY MONTH(order_purchase_timestamp))
        AS order_count,
       MONTH(order_purchase_timestamp) as month_number
FROM orders
...
query2018 <- "SELECT DISTINCT LEFT(MONTHNAME(order_purchase_timestamp), 3) as month,
       COUNT(order_id) OVER (PARTITION BY MONTH(order_purchase_timestamp))
        AS order_count,
       MONTH(order_purchase_timestamp) as month_number
FROM orders
...
queryAll <- "SELECT DISTINCT LEFT(MONTHNAME(order_purchase_timestamp), 3) as month,
       COUNT(order_id) OVER (PARTITION BY MONTH(order_purchase_timestamp))
        AS order_count,
       MONTH(order_purchase_timestamp) as month_number
FROM orders
...
sales_data2016 <- dbGetQuery(target_db, query2016)
sales_data2017 <- dbGetQuery(target_db, query2017)
sales_data2018 <- dbGetQuery(target_db, query2018)

all_months <- data.frame(month = factor(month.abb, levels = month.abb))

sales_data2016 <- all_months %>%
  left_join(sales_data2016, by = "month") %>%
  mutate(order_count = ifelse(is.na(order_count), 0, order_count))

sales_data2017 <- all_months %>%
  left_join(sales_data2017, by = "month") %>%
  mutate(order_count = ifelse(is.na(order_count), 0, order_count))

sales_data2018 <- all_months %>%
  left_join(sales_data2018, by = "month") %>%
  mutate(order_count = ifelse(is.na(order_count), 0, order_count))

sales_data2016$month <- factor(sales_data2016$month, levels = unique(sales_data$month))
sales_data2017$month <- factor(sales_data2017$month, levels = unique(sales_data$month))
sales_data2018$month <- factor(sales_data2018$month, levels = unique(sales_data$month))

par(mfrow = c(3, 1))

plot1 <- ggplot(sales_data2016, aes(x = month, y = order_count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Orders in 2016", x = "2016", y = "Order Count") +
  ylim(0, 8000)
# print(plot1)

plot2 <- ggplot(sales_data2017, aes(x = month, y = order_count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Orders in 2017", x = "2017", y = "Order Count") +
  ylim(0, 8000)
# print(plot2)

plot3 <- ggplot(sales_data2018, aes(x = month, y = order_count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Orders in 2018", x = "2018", y = "Order Count") +
  ylim(0, 8000)
# print(plot3)
...
```{r echo = FALSE, results = TRUE}
queryAll <- "SELECT DISTINCT DATE(order_purchase_timestamp) as day,
COUNT(order_id) OVER (PARTITION BY DATE(order_purchase_timestamp)) AS order_count
FROM orders
ORDER BY DATE(order_purchase_timestamp);"

sales_dataAll <- dbGetQuery(target_db, queryAll)
sales_dataAll$day <- as.Date(sales_dataAll$day, format = "%Y-%m-%d")

summary(sales_dataAll)

full_dates <- seq.Date(from = as.Date("2016-09-01"), to = as.Date("2019-01-01"), by = "day")

sales_dataAll <- sales_dataAll %>%
  complete(day = full_dates, fill = list(order_count = 0))

sales_dataAll <- sales_dataAll %>%
  filter(order_count != 0)

ggplot(sales_dataAll, aes(x = day, y = order_count)) +
  geom_point(aes(color = ifelse(order_count <= 100, "1-100", 
                                ifelse(order_count <= 500, "101-500", "501+")))) +
  scale_color_manual(values = c("1-100" = "red", "101-500" = "blue", "501+" = "black")) +
  theme(legend.position = "none") +
  labs(title = "Number of orders per day", x = "Sep 2016 - Dec 2018", y = "Order Count") +
  geom_vline(xintercept = as.Date("2016-10-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-01-01"), linetype = "dashed", color = "black") +
  geom_vline(xintercept = as.Date("2017-04-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-07-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-10-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2018-01-01"), linetype = "dashed", color = "black") +
  geom_vline(xintercept = as.Date("2018-04-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2018-07-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2018-10-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2019-01-01"), linetype = "dashed", color = "black") +
  scale_x_date(limits = c(as.Date("2016-09-01"), as.Date("2019-01-01")), 
               date_breaks = "1 year", date_labels = "%Y") +
  geom_hline(yintercept = 0, color = "black", linetype = "solid")
```

The plot reveals the following aspects of the data.  
a. The plot shows gaps in between dates when orders were placed.  
   Further analysis shows the gaps between dates in the dataset as follows:
...
b. The trend shoots up during the 4th quarter of 2017 and the order placed per  
   day sees a steep rise.  
c. One data point shows extreme deviation of 1176 orders on a single day on 24th  
   November 2017. This count deviated greatly from the Median of 148 orders and  
   Mean of 156 orders.
...
Hence we can conclude that the data reported through orders.csv is not conducive  
...
state_dataAll <- dbGetQuery(target_db, queryAll)

summary(state_dataAll)

ggplot(state_dataAll, aes(x = customer_state, y = customer_id_count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  theme(legend.position = "none") +
  labs(title = "Customers per State", x = "States", y = "Customer Count")
```
...
a. There is a high deviation in estimated delivery dates and actual delivery to the customer.  
   The estimated delivery dates require further analysis for more realistic estimates.  
b. There is a high deviation in delivery as per carrier and actual delivery to the customer.  
   Further investigation is required to drill into the issue.  
c. There is a high deviation in delivery as per carrier and estimated delivery to the customer.  
   Further investigation is required to drill into the issue.  
...
d. The following states require attention as they deviate from the average delivery time.  
   Further investigation is required to drill into the issue.
...
SELECT customer_state, total_orders, avg_delivered_customer AS avg_delivery_State,
       ROUND(avg_delivery_Country, 2) AS avg_delivery_Country
FROM d3
WHERE ((avg_delivery_Country) - (avg_delivered_customer)) < 0
ORDER BY total_orders DESC
```
e. The following bubble chart shows total orders vs average delivery time.  
   The size of the circle depicts order count. Colour gradient towards Blue indicates  
   better delivery time. Colour gradient towards Red indicates worse delivery time.  
   This chart will assist in identifying the states that requires attention to  
   improve customer experience by improving delivery time.
...
ggplot(state_dataAll, aes(x = customer_state, y = total_orders, size = total_orders, color = avg_delivered_customer)) +
  geom_point(alpha = 0.7) +
  # scale_size(range = c(5, 20), name = "Total Orders") +
  scale_size(range = c(5, 20), guide = "none") + 
  scale_color_gradient(low = "blue", high = "red", name = "Avg Delivery Time (days)") +
  scale_y_continuous(limits = c(0, 50000), breaks = seq(0, 50000, by = 5000)) +
  labs(title = "Total Orders and Average Delivery Time by State",
       x = "State",
       y = "Total Orders") +
  # theme_minimal() +
...
7.10. Analysis of products:

a. Top 2 selling products in each state is given by:  


```{sql echo=TRUE, connection=target_db}
WITH d AS (
    SELECT DISTINCT c.customer_state, p.`product category`,
           COUNT(`product category`) OVER (PARTITION BY customer_state, `product category`) 
              AS count_product
    FROM customers AS c
    JOIN orders AS o
    ON c.customer_id = o.customer_id
  ...
  d2 AS (
    SELECT d.customer_state, d.`product category`, d.count_product,
       DENSE_RANK() OVER (PARTITION BY d.customer_state ORDER BY count_product DESC)
          AS product_rank
    FROM d
    )
SELECT d2.customer_state, d2.`product category`, d2.count_product, d2.product_rank
FROM d2
WHERE d2.product_rank <= 2 AND d2.count_product > 500
ORDER BY d2.customer_state, d2.product_rank, d2.count_product DESC
```

b. Top selling products in the dataset are as follows:  

```{sql echo=TRUE, connection=target_db}
SELECT DISTINCT p.`product category` AS product_category,
       COUNT(oi.order_item_id) OVER (PARTITION BY `product category`) 
              AS count_product_category
FROM customers AS c
JOIN orders AS o
    ON c.customer_id = o.customer_id
JOIN order_items AS oi
...
c. Sales trend in the data set for top selling product category:  
...
WITH d AS (
  ...
  d2 AS (
    SELECT d.product_category, d.count_product_category,
           DENSE_RANK() OVER (ORDER BY d.count_product_category DESC) 
                AS rank_category
    FROM d
  ),
  top_prod_cat AS (
    SELECT d2.product_category
    FROM d2
    WHERE d2.rank_category = 1
 )
SELECT DISTINCT DATE(o.order_purchase_timestamp) as day,
       COUNT(o.order_id) OVER (PARTITION BY DATE(o.order_purchase_timestamp)) 
            AS order_count
FROM customers AS c
  JOIN orders AS o
        ON c.customer_id = o.customer_id
  ...
  JOIN products AS p
        ON oi.product_id = p.product_id
WHERE p.`product category` IN (SELECT product_category FROM top_prod_cat) 
...
ggplot(sales_dataAll, aes(x = day, y = order_count)) +
  ...
  theme(legend.position = "none") +
  # labs(title = "Number of orders per day", x = "Sep 2016 - Dec 2018", y = "Order Count") +
  labs(title = "Number of orders per day - bed table bath", x = "Sep 2016 - Dec 2018", y = "Order Count") +
  geom_vline(xintercept = as.Date("2016-10-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-01-01"), linetype = "dashed", color = "black") +
  geom_vline(xintercept = as.Date("2017-04-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-07-01"), linetype = "dashed", color = "grey") +
...
d. Sales trend in the data set for top 5 selling product category:  
...
ggplot(sales_dataAll, aes(x = day, y = order_count)) +
  ...
  theme(legend.position = "none") +
  # labs(title = "Number of orders per day", x = "Sep 2016 - Dec 2018", y = "Order Count") +
  labs(title = "Number of orders per day - Top 5 product categories", x = "Sep 2016 - Dec 2018", y = "Order Count") +
  geom_vline(xintercept = as.Date("2016-10-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-01-01"), linetype = "dashed", color = "black") +
  geom_vline(xintercept = as.Date("2017-04-01"), linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.Date("2017-07-01"), linetype = "dashed", color = "grey") +
...
```{r closeconnection, include=FALSE}
dbDisconnect(target_db)
```

____________________________________________________________________________________________________________________________________________________

**Programming environment for analysis of the dataset is as follows**

1. RStudio for markdown and plotting.  
2. MySQL as database server.  
3. Docker for hosting RStudio and MySQL server.  
4. Link to the [GitHub Repository for the case study](https://github.com/shayantandey/Scaler-DS-ML/tree/main/Business%20Case%20-%20SQL/Target%20Dataset).
